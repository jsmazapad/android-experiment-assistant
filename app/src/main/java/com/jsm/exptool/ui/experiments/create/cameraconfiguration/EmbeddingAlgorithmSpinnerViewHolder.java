package com.jsm.exptool.ui.experiments.create.cameraconfiguration;

import android.view.View;
import android.widget.TextView;

import com.jsm.exptool.R;
import com.jsm.exptool.core.ui.baserecycler.BaseRecyclerViewHolder;
import com.jsm.exptool.entities.embedding.EmbeddingAlgorithm;

public class EmbeddingAlgorithmSpinnerViewHolder extends BaseRecyclerViewHolder<EmbeddingAlgorithm> {


    TextView titleTV;
    TextView descriptionTV;

    protected EmbeddingAlgorithmSpinnerViewHolder(View v) {
        super(v);
        titleTV = v.findViewById(R.id.titleTV);
        descriptionTV = v.findViewById(R.id.descriptionTV);
    }

    @Override
    public void fillViewHolder(EmbeddingAlgorithm element) {
        titleTV.setText(element.getDisplayName());
        descriptionTV.setText(element.getDescriptionTranslatableRes());

    }
}
