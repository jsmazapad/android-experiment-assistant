...
public class Gyroscope extends MySensor implements SensorEventListener {
    ...
    public Gyroscope() {
        super(SensorConfigConstants.TYPE_GYROSCOPE, R.string.gyroscope, SensorConfigConstants.MIN_INTERVAL_MILLIS);
            ...
            public void run() {
                if (Gyroscope.this.isRecording) {
                    ...
                    if (BuildConfig.DEBUG)
                        Log.d("Sensor", App.getAppContext().getString(Gyroscope.this.getRName()) + ": " + values.toString() );
